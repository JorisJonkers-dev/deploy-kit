...
\textbf{Why UML class diagrams.} The question was why the metamodels are
documented with UML class diagrams rather than with a structural notation such
as a deployment or component diagram. The short answer is that the two are not
alternatives: a class diagram here is the notation for the \emph{abstract
syntax of a language}, whereas a deployment or component diagram describes
\emph{one system}. Section~\ref{sec:notation} gives the argument in full, and
distinguishes the metamodel's notation from the notation a user of the language
writes in.

\textbf{ArchiMate and C4.} Both were evaluated, and both influenced the domain
analysis without being adopted as the source language.
Section~\ref{sec:archimate} positions the source metamodel's concepts against
ArchiMate's technology and application layers and against the C4 model's
container level, which is a useful check that the language names the concepts
practitioners already name. Section~\ref{sec:alternatives} records why neither
is used as the source notation itself.
