...
% Each figure is cut from the one mermaid block that mirrors the metamodel, by
% scripts/diagrams/class-diagram.py, so no part can drift from the whole.
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.96\linewidth, keepaspectratio]{Figures/s-app}
...
Figures~\ref{fig:sourceprocess1} and~\ref{fig:sourceprocess2} show what hangs
off a \texttt{Process}. It is split in two because the whole is wider than a
printed page; both halves are cut from the same source as
Figure~\ref{fig:sourcemetamodel}, so no half can disagree with the whole.

\begin{figure}[p]
\centering
\includegraphics[angle=90, height=0.90\textheight, keepaspectratio]{Figures/s-proc1}
\caption{A Process: Capacity, Helpers, Health and What It Needs From a Machine}
\label{fig:sourceprocess1}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[angle=90, height=0.90\textheight, keepaspectratio]{Figures/s-proc2}
\caption{A Process: Storage, Configuration, Dependencies and the Ports It Offers}
