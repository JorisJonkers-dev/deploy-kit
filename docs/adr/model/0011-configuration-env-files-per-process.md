...
# Configuration is per-Process env files with named placeholders
...
Configuration is authored as env files in real dotenv format, **per Process**:
`platform/env/<process>/base.env` carries what does not vary, one overlay per
Cluster Target (`platform/env/<process>/<cluster>.env`) carries only what
...
another Application authored ([0018](0018-exposure-by-audience.md)). Writing a
derived value as a literal
is a build error; overriding a derived value uses the Process's declared
override mechanism ([0031](0031-derived-overrides-with-reason.md)), never the env
file. The old configuration ADR scoped these files per Application while the spec and
the worked examples (`knowledge-api.base.env`, `knowledge-ingest-worker.base.env`)
were per Process; the review caught the drift (finding X4) and this sentence
closes it, matching the per-Process identity of
[0024](0024-identity-per-process.md).

## Rests on

Every non-secret value a Process's environment needs is either application-owned (a
literal the author knows) or a total function of the Intent and pinned inputs
([0005](0005-derivation-is-total.md)). False if: a Process requires an
environment variable that is neither author-known nor producible by any
`${dependency:…}`, `${secret:…}` or `${exposure:…}` source, forcing a
hand-maintained copy of a derived value. Settled by: rendering the three project repositories' Processes
...
Configuration was nominally declared and actually hand-written. All three application
repositories ship a `platform/production.env` containing nothing but comments
(*"Non-secret production environment values… Rendered into the process fragment
...
Undo cost today: rewrite a handful of env files across three project repositories
...
- Sixty duplicated OTEL lines leave the project repositories; runtime boilerplate
...
- Coordinates shared between sibling Processes are written once per Process, not
  once per Application; placeholders cannot go stale: paid by application authors.
- Hand-written derived literals must be deleted before a repository's first
  render succeeds: paid by application owners at migration time.
