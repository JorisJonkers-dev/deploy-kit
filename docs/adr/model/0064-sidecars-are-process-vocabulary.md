...
# A Process may hold sidecars, and a sidecar carries what a container carries
...
must be placed differently from the Process it runs beside (which a shared pod
...
## Why
A Process holding more than one container is not an edge case here. It happens
...
agent-runner pods, not a process of its own"*, a real container excused from
...
`capabilities`) stay on the Process and describe the pod, while `memory`,
...
Eligibility then sums. A node must fit the Process's containers together, so
the placement check adds the sidecars' `memory` and `cpu` to the Process's
...
Application depends on, because the consumer's edge then resolves to a port the
Process's own container does not open, `platform-postgres`'s `metrics: 9187`
...
- Placement eligibility sums across containers, so a Process's node set narrows
  when a sidecar is added and a previously-placeable Process can become
...
- A surface may be served by a sidecar rather than by the Process's own
...
- Two more containers that must each meet `restricted` on their own, and a
  sidecar that cannot is refused along with its Process, paid in honesty:
  those containers were always running and were invisible.
- `probes` stay on the Process. A sidecar publishes no readiness signal of its
  own, so a failing exporter cannot hold its Process out of application, the
  ...
  Process's owner, who must notice.
