...
a restart. Settled by: deploy a process whose single health endpoint checks a
...
`port`, or `tcp` and a port for an application with no HTTP surface, and neither
...
liveness, which restarts the pod, converting a degraded application into a
...
`db` for both readiness and liveness, and the other data applications do the same.
...
|---|---|---|
| Keep the v2 fallback (`livenessPath` optional, defaults to `path`) | every application taking the default inherits the dependency-outage crash-loop; `app-ui` and `agents-login` already do | makes the worst wiring the path of least resistance |
...
| HTTP-only probes, no `tcp` | `postgres` and the rest of the data tier need fake HTTP sidecars or lose probes entirely | the estate's stateful applications are TCP-native today |
...
Undo cost today: restore the fallback in the probe derivation (the process
...
one rolling restart per process whose rendered probes change.
Becomes irreversible once: processes ship liveness endpoints distinct from
their readiness paths. Reinstating a fallback would then silently repoint
liveness for every application that omits it, and no diff would show which
applications regressed.

## Consequences

- Every HTTP process authors two endpoint declarations even when they are
  deliberately identical, paid by application authors.
...
- A dependency outage degrades an application to unready instead of restarting it,
  provided the authored liveness endpoint checks process health only, paid by
  application authors, in endpoint discipline.
- Port-less workers add one `probes: none` line so silence is never ambiguous,
  paid by their authors.
- Adapters render `httpGet` and `tcpSocket` variants and refuse a Process
...
- Pointing both probes at one endpoint remains expressible; the model makes it
  visible, not impossible, paid by the application that chooses it.
