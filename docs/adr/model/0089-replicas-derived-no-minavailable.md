...
claim: settled
date: 2026-09-07
normative: spec/v1/10-project-intent.md#replicas-and-the-disruption-budget
rests-on: ["0002"]
---
...
> ([chapter 10](../../../spec/v1/10-project-intent.md#capacity)).

## Rests on
No Process in this estate obtains availability from a replica count, so a
declared availability requirement could only ever be a request the substrate
cannot honour. False if: a stateless Process's second replica measurably
survives an event that takes the first one down: which requires two nodes, a
shared-nothing process and a load balancer that notices. Settled by: rendering
...
does not survive its node, so every stateful Process is pinned to one machine
...
single-replica Process gets no budget, because a budget that forbids its only
eviction is not protection.

Emitting no PDB at all was tempting and slightly wrong: a two-replica stateless
Process spread over seven nodes does benefit from not losing both at once, and
...
| `minAvailable: replicas - 1` | Stays in the vocabulary the live objects use | The arithmetic is per Process, so a count change silently changes the budget's meaning, and `replicas: 1` still deadlocks unless special-cased |
| No PDB at all | Nothing to derive; every stateful Process is pinned anyway | Loses the one case a budget earns here: a multi-replica stateless Process losing both pods to one drain |
...
- R16 closes, and the drain deadlock closes with it: no rendered PDB can forbid
  the eviction of a Process's only pod, paid by nobody, and it was reachable
...
- The estate's six live PDBs are not all re-derivable: any over a single-replica
  Process will not be rendered, so adoption drops them, which is the intended
