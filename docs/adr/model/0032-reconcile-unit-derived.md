...
Applications implies, and removing that edge breaks a deploy. Settled by: derive the
unit and order for every first-party Application and diff the derived graph against
...
types it `"type": "string"` with no enumeration, and every Application declared
...
noticed, because it feeds application-registry registration and never placement. A
...
application-level field cannot describe the cluster even in principle: one string
cannot name two units. The removal is therefore from the layer-1 authoring
vocabulary, `platform.layer` is deleted from Project Intent, and it extends to
...
of each observed application, and it leaves that list. What remains is optional and
...
`dependsOn` graph is the application dependency graph of
[0020](0020-dependency-edges-carry-surface.md) projected onto Projects:
...
again, a day of schema and validator work plus one pull request per application
...
## Consequences
- An application owner cannot pin their reconcile position; a wrong order is fixed by
  correcting the dependency declaration that produced it, paid by the Application
...
- `layer` leaving the cluster-state `required` list is a breaking change for the
  one reader that indexes on it, the application registry, paid by that reader's
  owner.
...
- An Application's objects may split across units with no declaration saying so, as
  `agents-login`'s do, paid by whoever debugs a partially-applied Application.
