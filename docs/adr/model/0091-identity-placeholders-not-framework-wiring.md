...
# The model derives no framework wiring; it exposes the Process's own identity as placeholders
...
## Rests on
A self-delivering Process's Vault client configuration is its framework's
concern, and the only part the model must supply is the derived values that
configuration references. False if: two Processes of the estate wire the same
...
whenever a repository changes library. [0078](0078-engine-is-process-vocabulary.md)
...
So the wiring stays where the framework knowledge is: the Process's own env
...
Process writing its namespace as a literal, which is the same defect one field
later. Reusing `${dependency:…}` for self-reference would muddle a source that
means "an edge to another Application" and force the edge invariants to special-case
it.
...
| Only `${identity:vaultRole}` | Smallest surface, nothing speculative | The next Process writes its namespace as a literal, reintroducing the drift one field over |
| Reuse `${dependency:…}` for self-reference | No new source | A dependency is an edge to another Application; self-reference muddles the source and the invariants that check edges |
