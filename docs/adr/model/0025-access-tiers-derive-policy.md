...
claim: settled
date: 2026-08-31
normative: spec/v1/10-project-intent.md#access-tiers
rests-on: ["0009"]
---
...
Every secret-touching process in the estate holds exactly one of four
intents (`read`, `self-renew`, `self-roll`, `custody`), and the least
privilege each needs follows from the intent alone. False if: a process needs
a capability set no tier's derivation produces, or two processes on the same
...
| `custody` as an enumerated path list | the grant stays a path grant, so [0023](0023-grant-unit-is-the-path.md) needs no prefix case and roll-impact stays exact | `agents-api` mints paths keyed by runtime ids; enumeration would require a control loop rewriting Application documents from cluster state, which inverts the authoring direction |
...
in the estate's Application documents; every rendered Vault policy re-renders.
Becomes irreversible once: production tokens authenticate against derived
policies and the `self-renew` processes run with no Vault privilege at all.
...
- A self-renewing process gets an identity with no capability on its path,
...
- Four tiers times three deliveries is twelve cells and not all are legal, so
  ...
  application authors and by chapter 10, which keeps the table current.
