...
## Rests on
Every cross-application connection in the live estate is expressible as a triple:
provider Application Id, one surface that provider already declares, and whether the
...
Application under a single consumer reference. Settled by: extract every host:port
literal and every endpoint-shaped environment key from the first-party
processes' env files and configuration, and match each to a `provides` entry on
the Application that owns that address, any reference matching no declared surface
is a counterexample.
...
nor a coordinate. That value has to live somewhere, and [0011](0011-configuration-env-files-per-process.md)
...
`5432`, and the Application Id stays the only referencable identity
([0010](0010-flat-application-identity.md)). That single declaration is then enough
...
processes, so the cluster is effectively open east-west, and no default-deny
...
| Consumer restates the port: `{application: platform-postgres, port: 5432}` | One extra literal per edge; ~30 processes to update whenever a provider moves a port | Duplicates the provider's own declaration in every consumer, so a port change is an estate-wide edit that no tool can verify |
...
Application schema plus the four derivation sites that read them; with no production
...
every Application already migrated off a hand-written endpoint literal has to have
...
shape re-renders every process's policy at once, and the only safe path back is
...
- Credential-free dependencies become declarable and therefore visible; the
  ...
  paid by every Application relying on an undeclared path.
