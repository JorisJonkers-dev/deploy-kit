...
# The foundation is declared as Applications; nothing hand-written enters the render
...
## Rests on
Every foundation component the estate runs can be stated in the Project Intent
vocabulary (an image, Processes, grants, exposure, volumes, a class) and what
...
project files and diffing the result against what the packs deliver today, with
...
Process with a volume of class `irreplaceable` and an `engine`. Traefik is two
Applications placed by capability (one on the `public-ingress` node, one on a LAN
node), each the proxy for one tier. VSO is a Process with grants. Prometheus and
Gatus are Processes whose configuration is an **inbound derivation**: what every
...
itself, the Flux source and Vault's unseal. Adding a chart-shaped Process source
...
CRDs. Growing it is a decision. Everything else is an Application.

Deleting the pass-through is the same decision applied to the escape hatch.
[0012](0012-assets-not-code.md) refuses executable content for an Application; a raw
...
| A chart-shaped Process source rendering `HelmRelease` | Fast path for chart-only upstreams | Reintroduces a values surface no invariant reads, and the Deployment-shaped checks see nothing |
...
## Reversibility
Undo cost today: the platform project files are deletable and the packs still
...
- `flux-packs` and `flux-source` are deleted, and `HelmRelease` and
  ...
  foundation Applications, once each.
...
- Adoption order in chapter 60 puts the platform project first, because tenants
