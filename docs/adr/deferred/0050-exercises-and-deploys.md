...
Application's pre-deploy gate runs every Aggregator naming it, so overlap is the
...
tries to apply an Application it does not own receives a 403 rather than producing a
...
every kind and no `resourceNames`, `aliases.namespace` lets two Applications share a
...
test classes between them, so `media-stack` deploys seven applications behind smoke
...
- An Application can be gated by every relationship it participates in at no
  coordination cost, paid by Aggregator authors, maintaining two alike lists.
- A new Application is undeployable until exactly one Aggregator claims it, failing
  as `E_NO_DEPLOYER` at composition, paid by the Application owner, at creation.
- Every project needs a default Aggregator whether a relationship justifies one or
  not, paid by the platform owner, once per untested project.
