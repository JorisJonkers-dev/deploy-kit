...
nothing outside the Applications that Aggregator deploys (the Secret reach `create`
...
still admits two Applications with different deployers into one namespace. Settled
...
"a workflow that tries to apply an Application it does not own receives a 403 rather
...
exists precisely to let Applications share a namespace, so that is the sanctioned
case, and `E_MULTIPLE_DEPLOYERS` cannot see it: the invariant
(`spec/v1/40-composition.md:193`) asks whether an Application has two deployers, not
...
a namespace change is a DNS change (`<application>.<namespace>.svc`), so consumers,
dependency edges and NetworkPolicies follow, and the blast radius is exactly the
Applications sharing a namespace across deployers, today `app-system`. Becomes
irreversible once: those Applications have moved and are addressed at the new names.

## Consequences

- `aliases.namespace` narrows to a rename inside one deployer's own namespaces,
  and Applications sharing one across deployers must move, paid by joris, once.
- The namespace-wide Secret read is contained, not removed: a compromised deploy
  workflow still gets every Secret its own Applications hold, and withholding
