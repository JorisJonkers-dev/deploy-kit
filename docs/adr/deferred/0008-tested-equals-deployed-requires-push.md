...
reconciles. False if: the wired suite can gate the same lock Flux deploys, the lock is tested in CI, and the merge that passes gates the Flux source
update, giving tested-equals-deployed with no push machinery. Settled by:
[workspace#45](https://github.com/JorisJonkers-dev/workspace/issues/45), write the missing workflow caller, run the existing suite against the existing
