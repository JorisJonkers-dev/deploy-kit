...
| REQ-031 | A Platform document and the Project documents read with it are checked together, and a tier proxy, audience, engine, durability class, at-rest secret delivery, migration runner, delivery policy or handover path the Platform cannot satisfy is refused in the document that must change | [test/model/intent-set.test.ts](../test/model/intent-set.test.ts) |
