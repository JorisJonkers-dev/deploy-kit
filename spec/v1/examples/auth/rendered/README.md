...
| `apps/auth/servicemonitor.yaml` | `observability runner` | `scrape {port, path}`, `provides` (8081 → port name `http`) | cadence from the runner configuration (`runner.config.yml`, [G-17](#g-17) closed); the operator's `release` selector label ([G-11](#g-11)) |
...
| `observability/prometheusrules.yaml` | `observability runner` | the baseline rule set for `auth-api`'s scrape surface, with severity and receiver from `alertClass: page` | — (0079) |
...
| <a id="g-17"></a>G-17 | **No scrape `interval` or `scrapeTimeout`.** Omitted, which silently takes whatever the metrics stack's global default is — a value decided outside the model. **Closed by the observability boundary**: cadence now lives in the runner configuration ([`../observability/runner.config.yml`](../../observability/runner.config.yml)) and the tree's monitors are its output |
