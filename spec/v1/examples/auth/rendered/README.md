...
| `apps/auth/servicemonitor.yaml` | `prometheus` | `observability.scrape {workload, surface, path}`, `provides` (the `http` surface) | cadence from the Platform document ([G-17](#g-17) closed); the operator's `release` selector label ([G-11](#g-11)) |
...
| `apps/vso-secrets/policies/auth-api.policy.json` | `vault-policy` | the Workload's grants and their access tiers, per engine: KV read plus its `metadata` sibling, `transit/sign` and `transit/keys/.../rotate` for the JWT key | none (0073, 0085, 0086) |
