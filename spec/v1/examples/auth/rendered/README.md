...
| `apps/auth/servicemonitor.yaml` | `prometheus` (a `PodMonitor`) | `observability.scrape {process, surface, path}`, `provides` (the `http` surface) | cadence from the Platform document ([G-17](#g-17) closed); the operator's `release` selector label ([G-11](#g-11)) |
...
| `apps/auth/canary.yaml` | `kubernetes` | `cutover: continuous` on both Processes, `replicas` on auth-api (an autoscaler with equal bounds), the Platform document's `delivery.analysis`, the Application revision | none; the Services and the primaries are Flagger's ([chapter 30](../../../30-deliverables.md#flagger-ready-objects)) |
