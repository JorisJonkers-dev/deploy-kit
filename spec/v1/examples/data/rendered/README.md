...
| `apps/platform-postgres/servicemonitor.yaml` | `observability runner` | `scrape {port, path}`, `provides` (9187 → `metrics`) | cadence from the runner configuration (`runner.config.yml`, [G-19](#g-19)); the operator's `release` selector ([G-21](#g-21)); a PrometheusRule from `alertClass: page` ([G-33](#g-33)) |
...
| `apps/platform-rabbitmq/servicemonitor.yaml` | `observability runner` | `scrape`, `provides` (15692 → `metrics`) | cadence from the runner configuration; a rule from `alertClass: urgent` ([G-33](#g-33)) |
...
| `observability/prometheusrules.yaml` | `observability runner` | the baseline set per Service plus the engine rules the catalog carries for postgres and rabbitmq; severity and receiver from each Service's own class | — (0079) |
...
| `PrometheusRule`, anywhere | The observability service owns it, not the model — [G-33](#g-33) |
...
| <a id="g-19"></a>G-19 | **No scrape `interval` or `scrapeTimeout`.** Omitted on both ServiceMonitors, which silently takes the metrics stack's global default — a value decided outside the model. **Closed by the observability boundary**: cadence now lives in the runner configuration ([`../observability/runner.config.yml`](../../observability/runner.config.yml)) |
...
| <a id="g-33"></a>G-33 | **`alertClass` renders nothing in this tree, and that is now correct.** Three Services in one file declare three different values — `page`, `urgent`, `none` — kept per Service precisely so the domain does not page as loudly as its loudest member, and so a Service with no signal can say so. None of the three renders a monitoring object here, because the model no longer owns one: the class and the `scrape` facts leave the model and the observability service's runner turns them into monitors, rules, severity and receiver routing ([chapter 10](../../../10-service-intent.md#the-observability-boundary)). That boundary is the answer to a specific hole rather than a preference: in the generation being replaced the class was supposed to derive objects inside the model and derived none — no registered adapter rendered a `PrometheusRule` (zero occurrences under `src/`, either generation), the Gatus `alerts` block carried no mapping from an Alert Class to a receiver, threshold or send-on-resolved, and the notifier route from `alertClass` + `owner` had no producer at all. What the model still guarantees is the part that failed: `platform-postgres` declares `page`, the loudest class in the estate, on the Service eight others queue behind, and Gatus derives from `exposure` while a datastore is correctly not exposed — so its signal is the exporter sidecar's `scrape` surface and nothing else. Without that surface the declaration is `E_ALERT_CLASS_WITHOUT_SIGNAL` at composition, which is what `platform-valkey` runs into and why it declares `none` |
