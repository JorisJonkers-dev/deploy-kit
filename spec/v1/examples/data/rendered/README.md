...
| `apps/platform-postgres/workload.yaml` | `kubernetes` | `lifecycle`, `image`, `runtime`, `provides` (×2 surfaces), `placement` (memory, cpu, arch, disk), `hardening` + its exception, `sidecars`, `probes` (tcp), `startupBudget`, `zeroDowntime`, `stateful`, `volumes`, `assets`, env file | the digest and repository path ([G-03](#g-03)); whether `stateful` means StatefulSet ([G-04](#g-04)); `replicas` ([G-05](#g-05)); a node label for `disk` ([G-06](#g-06)); the PV binding that actually places it ([G-07](#g-07)); the Asset's content hash ([G-08](#g-08)); which container gets which env key ([G-09](#g-09)); everything the ungraded `sidecars` field leaves open ([G-02](#g-02)); a UID and an fsGroup ([G-15](#g-15)); where the hardening controls land ([G-16](#g-16)); the label set ([G-13](#g-13)) |
...
| `apps/platform-postgres/servicemonitor.yaml` | `kubernetes` | `scrape {port, path}`, `provides` (9187 → `metrics`) | `interval` / `scrapeTimeout` ([G-19](#g-19)); the operator's `release` selector ([G-21](#g-21)); a PrometheusRule from `alertClass: page` ([G-33](#g-33)) |
...
| `apps/platform-rabbitmq/workload.yaml` | `kubernetes` | `lifecycle`, `image`, `runtime`, `provides` (×3), `placement` (memory, cpu), `hardening`, `probes`, `startupBudget`, `zeroDowntime`, `stateful`, `volumes` | the digest ([G-03](#g-03)); object kind ([G-04](#g-04)); `replicas` ([G-05](#g-05)); that the locked digest may not run on 3 of its 7 eligible nodes ([G-27](#g-27)); UID/fsGroup ([G-15](#g-15)); its env file, which the example set omits ([G-28](#g-28)) |
...
| `apps/platform-rabbitmq/servicemonitor.yaml` | `kubernetes` | `scrape`, `provides` (15692 → `metrics`) | `interval` ([G-19](#g-19)); a rule from `alertClass: urgent` ([G-33](#g-33)) |
...
| `apps/platform-valkey/workload.yaml` | `kubernetes` | `lifecycle`, `image`, `runtime`, `provides`, `placement` (memory, cpu), `hardening`, `probes`, `startupBudget`, `zeroDowntime`, `stateful`, `volumes` | the digest ([G-03](#g-03)); object kind ([G-04](#g-04)); `replicas` ([G-05](#g-05)); architecture vs digest ([G-27](#g-27)); its env file ([G-28](#g-28)) |
...
| `servicemonitor.yaml` for valkey | No `scrape`. No exporter runs beside it, so there is no metrics surface and none is invented |
...
| `PrometheusRule`, anywhere | Three Services, three Alert Classes, no producer — [G-33](#g-33) |
...
- the health timeout class is taken as *the strongest class across a Service's
  ...
  not surface — it will on the first domain that mixes classes.
...
`platform-postgres` declares `stateful: true`, `zeroDowntime: false`, and one
...
- **`Recreate` is forced, not chosen.** A ReadWriteOnce `local-path` volume
  ...
  `Recreate` side. `zeroDowntime: false` records the same fact from the author's
  side, and the two agree here — nothing checks that they always will. The
...
- **`StatefulSet` buys nothing available here.** Its distinguishing feature is
  `volumeClaimTemplate`, which chapter 10 forbids outright — a template ties the
...
| <a id="g-02"></a>G-02 | **`sidecars` is ungraded and this domain's metrics surface depends on it.** `postgres-exporter` is rendered from a proposed field (chapter 10, "Still to be graded" item 1). Open in the rendered object: it has **no `resources`**, because `placement` is per Workload — so it is BestEffort inside a Burstable pod, and the eligibility check that matched 2Gi against a node accounted for one of two containers; **no probes**, because `probes` names port 5432 which it does not open; **no `hardening` of its own**, so it takes the unrelaxed class while its sibling takes the exception, which is a serialisation choice; and **no identity**, so a grant scoped "to the exporter" is in practice held by the database container too. The sharpest consequence is the restart target: `rotation: {tolerates: restart}` on the exporter's grant derives `{kind: Deployment, name: postgres}`, which under `Recreate` takes the datastore down to rotate a read-only connection string. A sidecar-scoped restart target is not expressible |
...
| <a id="g-09"></a>G-09 | **One env file, two containers, no partition rule.** Env files are per **Workload** and this Workload has two containers. `POSTGRES_DB` / `POSTGRES_USER` belong to the database; `DATA_SOURCE_NAME` belongs to the exporter. Nothing partitions them, so the whole file reaches both — the database container holds the exporter's connection string in its environment, which is a real widening inside the pod and the mirror image of the split-path work that produced the grant |
...
| <a id="g-19"></a>G-19 | **No scrape `interval` or `scrapeTimeout`.** Omitted on both ServiceMonitors, which silently takes the metrics stack's global default — a value decided outside the model |
