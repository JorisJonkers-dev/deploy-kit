...
code for anything but hashing, or the transformation's output serialises to a
`resolved.json` different from the committed one. Settled by: the resolved
parity suite green for every case under `spec/v1/examples/`.
...
- A derivation the transformation lacks shows up as a resolved parity failure,
  not a silent default. Paid by the author of the derivation.
